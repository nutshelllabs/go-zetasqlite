...
// boolTaggingFuncs are the functions that either serialize a value or store it
// in a container, so a BOOL argument has to keep its type rather than arriving
// as the INT64 that SQLite hands back. Struct fields are tagged separately in
// extractMakeStructData, which has the field types.
var boolTaggingFuncs = map[string]bool{
	...
	"zetasqlite_make_array":          true,
	"zetasqlite_array_agg":           true,
}
