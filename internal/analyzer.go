  ...
  if recursionDepth > 10 {
    ...
    if err != nil {
      if len(declarationMap) > 0 {
        ...
        if parsedErr.TermLength > 0 {
          // substitute and try again
          functions, err := a.AnalyzeR(ctx, conn, fixDeclarationError(query, parsedErr, declarationMap[parsedErr.IncompleteColumnName]), args, recursionDepth + 1)
